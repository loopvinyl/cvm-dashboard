...
def simular_investimento(ticker, data_inicio, valor_investido=1000):
    ...
    CORRIGIDA: compatibilidade de timezone e tipo de data
    """
    try:
        ...
        # CORREÇÃO: Converter data_inicio para datetime (sem timezone)
        # Garante compatibilidade entre datetime.date (st.date_input) e datetime.datetime
...
if modo_analise == "🏆 Dados Gerais":
    ...
    with rank_tab5:
        ...
        dy_results = []
        st.info(f"Buscando Dividend Yield para {len(tickers_para_rank)} empresas...")
        dy_progress = st.progress(0)
        
        for i, ticker in enumerate(tickers_para_rank):
            dy = calcular_dividend_yield(ticker)
            if dy is not None:
                # Busca o setor para exibir no ranking
                setor = df_filtrado[df_filtrado['Ticker'] == ticker]['SETOR_ATIV'].iloc[0]
                dy_results.append({'Ticker': ticker, 'SETOR_ATIV': setor, 'Dividend Yield': dy})
            dy_progress.progress((i + 1) / len(tickers_para_rank))
        ...
        if not df_dy_ranking.empty:
            ...
            # Tabela formatada
            df_dy_ranking['Dividend Yield'] = df_dy_ranking['Dividend Yield'].apply(
                lambda x: formatar_percentual_brasil(x)
            )
            st.dataframe(df_dy_ranking[['Ticker', 'SETOR_ATIV', 'Dividend Yield']], use_container_width=True)

        else:
            st.info("ℹ️ Não foi possível calcular dividend yields para as empresas selecionadas.")
