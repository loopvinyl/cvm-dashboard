# ==============================================================
# 📊 DASHBOARD CVM - Indicadores Financeiros (VERSÃO COMPLETA COM DADOS LOCAIS)
# ==============================================================
...
def carregar_dados_acoes_completo():
    """
    Carrega dados completos de ações do arquivo Excel
    """
    try:
...
def buscar_dividendos_historicos(ticker):
    """
    Busca dividendos históricos usando o arquivo Excel
    """
    try:
        ...
        # Ordenar por data
        dividendos_ticker = dividendos_ticker.sort_values('Data')
        dividendos_ticker['Ano'] = dividendos_ticker['Data'].dt.year
        dividendos_ticker['Mes'] = dividendos_ticker['Data'].dt.month
        
        return dividendos_ticker
...
def calcular_estatisticas_dividendos(df_dividendos):
    """
    Calcula estatísticas dos dividendos a partir do Excel
    """
...
def simular_investimento_lotes(ticker, data_inicio, quantidade_acoes=100):
    ...
    CORREÇÃO COMPLETA: Compatibilidade de datas entre Excel e datetime
    """
    try:
        # Buscar histórico de preços do Excel
        historico = buscar_historico_precos(ticker, "max")
        if historico is None:
            st.warning(f"❌ Não há dados de preços para {ticker}")
            return None
        
        # Buscar dividendos do Excel
...
def buscar_cotacao_atual(ticker):
    ...
    try:
        ...
        return {
            ...
            'market_cap': None,
            'volume': None,
            'data_atualizacao': ultima_cotacao['Data'].strftime("%d/%m/%Y")
        }
...
def calcular_dividend_yield_otimizado(ticker, periodo_anos=1):
    """
    Versão otimizada do cálculo de dividend yield usando Excel
    """
    try:
        # Buscar cotação atual do Excel
...
def calcular_ranking_dividendos(df_filtrado, periodo_anos=1, limite_empresas=50):
    """
    Calcula ranking de dividendos com diferentes períodos usando Excel
    """
...
if DADOS_ACOES is not None:
    ...
    st.sidebar.warning("⚠️ Dados de ações não encontrados")
...
if modo_analise == "🏆 Dados Gerais":
    ...
    with rank_tab5:
        ...
        if DADOS_ACOES is None:
            st.error("""
            **❌ Arquivo de dados de ações não encontrado**
            
            Para usar esta funcionalidade, você precisa do arquivo:
            **`historico_acoes_completo.xlsx`**
            
            Este arquivo deve conter:
            - Planilha 'dividendos' com colunas: Ticker, Data, Dividendo
            - Planilha 'cotacoes' com colunas: Ticker, Data, Fechamento/Close
            
            **📊 Dados Disponíveis no Arquivo:**
            - 8,468 registros de dividendos históricos
            - 266 tickers únicos com dados de dividendos  
            - Período completo de 2000 a 2025
            - Dados de empresas como PETR4, VALE3, ITUB4, etc.
            """)
        else:
            ...
            if dados_dy:
                ...
                **📈 Interpretação:**
                ...
                st.warning("Não foi possível calcular os dividend yields. Verifique se os dados estão disponíveis para as empresas selecionadas.")
        ...
        with tab_dividendos:
            ...
            if df_dividendos is not None and not df_dividendos.empty:
                ...
                **ℹ️ Dados de Dividendos Não Encontrados**
                ...
                - Dados não disponíveis no arquivo Excel
            ...
            if st.button("🎯 Calcular Simulação", type="primary"):
                ...
                if resultado:
                    ...
                    **❌ Não foi possível realizar a simulação**
                    ...
                    - Erro nos dados do arquivo Excel
...
with st.sidebar.expander("💡 Metodologia livro Vellani (2024)"):
    ...
    **Valuation Lucro Econômico/SELIC (CORRIGIDO):**
    ...
    - **COTAÇÃO ESPERADA:** Valor da Empresa (R$) ÷ Número de Ações
    - **COTAÇÃO:** Busca em tempo real via arquivo Excel
    - **ANÁLISE:** Comparação entre valuation calculado e cotação de mercado

    **Novas Funcionalidades:**
    - **DADOS LOCAIS:** Usa arquivo Excel com 8.468 dividendos e 911.007 cotações
    ...
    **Dataset: historico_acoes_completo.xlsx**
    - Dividendos: 8.468 registros (2000-2025)
    - Cotações: 911.007 registros (2010-2025)
    - Tickers únicos: 266 (dividendos), 293 (cotações)
    - Dados completos de empresas como PETR4, VALE3, ITUB4, etc.
    """)
