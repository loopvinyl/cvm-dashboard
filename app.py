...
def simular_investimento_lotes(ticker, data_inicio, quantidade_acoes=100):
    ...
    CORRIGIDA: compatibilidade de timezone e dtype
    """
    try:
        ...
        # **ALTERAÇÃO PARA COMPATIBILIDADE DE DATA:** # Converter data_inicio para datetime.datetime (sem timezone) para garantir que
        # a comparação com o DatetimeIndex do 'historico' não falhe (dtype=datetime64[ns] vs date)
        if isinstance(data_inicio, str):
            data_inicio = datetime.strptime(data_inicio, '%Y-%m-%d')
        elif isinstance(data_inicio, datetime.date): # Captura o caso de st.date_input retornar date
...
- **Entre 4%-8%:** Yield atrativo - bom para renda...
            ...
            st.warning("Não foi possível calcular o ranking de Dividend Yields. Verifique a conexão com o Yahoo Finance ou os tickers disponíveis.")
...
# ==============================
    ...
    with tab4:
        ...
        with col_data:
            ...
            data_compra_simulacao = st.date_input("Data da compra", value=datetime(2015, 1, 1).date(), min_value=datetime(2000, 1, 1).date(), max_value=datetime.now().date() - timedelta(days=365))
